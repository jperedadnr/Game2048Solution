 ...
 */
public class NTupleBoard implements IStateNTuple {
     ...
     * @param tiles
     */
    public NTupleBoard(SimpleTile[] tiles) {
        iWin = false;
        canMove = true;
        this.tiles = tiles;
        partialScore = 0;
     ...
     */
    public void setTiles(SimpleTile[] tiles) {
        this.tiles = tiles;
    }
     ...
     */
    public boolean isEqual(NTupleBoard gameBoard) {
    ...
    private boolean calculateCanMove() {
        ...
        for ( int x = 0; x < 4; x++ ) {
            for ( int y = 0; y < 4; y++ ) {
                ...
                if ( (x < 3 && t.getValue() == this.tileAt(x + 1, y).getValue())
                        || ((y < 3) && t.getValue() == this.tileAt(x, y + 1).getValue()) ) {
                    return true;
                }
            }
        }
